...
def Q(A:list):
    '''Returns an orthonormal list of vectors that shares the same span as the input matrices columns
        ...
        call stable gram schmidt and return the first element from that output
...
#Matrix Sum
def Msum(m):
    '''Adds together all elements of a matrix
        
        set a temperary value to zero
        iterate through the matrix and add each element to that variable
        if there is a complex number add the real and the imaginary part
        
        Args:
            m : a matrix (list of lists of floats)
            
        Returns:
            A singular float that is the sum of all the elements in the matrix
    '''
    Sum:float = 0
    for i in m:
        for j in i:
